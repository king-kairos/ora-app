...
async function walk(dir: string, results: string[] = []) {
  ...
  for (const item of list) {
    ...
    if (item.isDirectory()) await walk(full, results);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!search) {
      return NextResponse.json(
        { ok: false, error: "search requerido" },
        { status: 400 }
      );
    }
    ...
    const backupRoot = path.join(ROOT, "ora-backups", "multi-context", stamp());
    ...
    for (const file of files) {
      ...
      try {
        ...
        const lineIndex = lines.findIndex((l) => l.includes(search));
        ...
        // ignorar archivos que no se puedan leer como texto
      }
    }
    ...
    return NextResponse.json({
      ok: true,
      engine: "MULTI_CONTEXT_PATCH_ENGINE_V2",
      dryRun,
      total: matches.length,
      matches,
      changedFiles,
      build,
      backupRoot: dryRun ? null : path.relative(ROOT, backupRoot),
    });
  } catch (error: any) {
    return NextResponse.json(
